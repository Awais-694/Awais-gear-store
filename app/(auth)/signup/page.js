...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatusMsg({ type: "error", text: "Network connection to signup API failed." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative w-full flex items-center justify-center py-4 px-4 overflow-hidden">
      ...
      <div className="w-full max-w-sm bg-white/95 backdrop-blur-md border border-gray-150 p-6 rounded-[1.5rem] shadow-[0_15px_40px_-12px_rgba(0,0,0,0.06)] relative z-10 overflow-hidden transition-all duration-300">
        ...
        <header className="mb-5 text-center">
          ...
          <p className="text-xs text-gray-500 mt-1">Sign up to access custom tech gear</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-gray-700 uppercase tracking-wider mb-1 pl-1">Full Name</label>
            <input
              type="text"
              required
              placeholder="Awais Saboor"
              value={formData.name}
        ...
        <button 
          type="button" 
          onClick={() => alert("Social signup is currently disabled for this demo.")}
        ...
        <p className="text-xs text-center text-gray-500 mt-5">
          ...
          <Link href="/login" className="text-blue-600 font-extrabold hover:text-blue-700 transition-colors">
            Sign In here
          </Link>
        </p>
      </div>
